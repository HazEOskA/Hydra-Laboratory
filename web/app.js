...
const HYDRA_CONFIG = Object.freeze({
  apiEnabled: Boolean(window.HYDRA_CONFIG?.apiEnabled),
  apiBase: String(window.HYDRA_CONFIG?.apiBase || "").replace(/\/+$/, ""),
  preview: Boolean(window.HYDRA_CONFIG?.preview),
});
...
async function api(path, options = {}) {
  if (!HYDRA_CONFIG.apiEnabled) {
    throw new Error("HYDRA runtime API is not connected in this preview.");
  }
  const headers = { Accept: "application/json", ...(options.headers || {}) };
  if (options.body !== undefined) headers["Content-Type"] = "application/json";
  const response = await fetch(`${HYDRA_CONFIG.apiBase}${path}`, { ...options, headers });
...
function renderMissionRail() {
  ...
  append(form, failure, button(
    HYDRA_CONFIG.apiEnabled ? "Compile mission" : "Runtime not connected",
    "primary-button full-width",
    null,
    {
      ariaLabel: HYDRA_CONFIG.apiEnabled ? "Compile local mission" : "Runtime API not connected",
      type: "submit",
      disabled: !HYDRA_CONFIG.apiEnabled,
      title: HYDRA_CONFIG.apiEnabled ? "Compile local mission" : "Connect the reviewed Hydra runtime API to enable mission execution",
    }
  ));
...
if (HYDRA_CONFIG.apiEnabled) {
  ...
  dom.emergency.disabled = true;
  dom.emergency.title = "Runtime API not connected in this preview";
  render();
}
