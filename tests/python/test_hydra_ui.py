...
class HydraCommandCenterCase(unittest.TestCase):
    ...
    def test_shell_and_local_assets_are_served_with_security_headers(self) -> None:
        ...
        status, body, headers = self.get("/runtime-config.js")
        self.assertEqual(status, 200)
        self.assertIn(b"apiEnabled: true", body)
    ...
    def test_mission_intake_uses_a_real_submit_control(self) -> None:
        ...
        self.assertIn('HYDRA_CONFIG.apiEnabled ? "Compile mission" : "Runtime not connected"', source)
        self.assertIn('type: "submit"', source)
        self.assertIn("disabled: !HYDRA_CONFIG.apiEnabled", source)

    def test_vercel_preview_fails_closed_when_runtime_is_absent(self) -> None:
        config = (REPO_ROOT / "web" / "runtime-config.js").read_text(encoding="utf-8")
        source = (REPO_ROOT / "web" / "app.js").read_text(encoding="utf-8")
        self.assertIn("apiEnabled: false", config)
        self.assertIn("HYDRA runtime API is not connected in this preview.", source)
        self.assertIn("dom.emergency.disabled = true", source)
